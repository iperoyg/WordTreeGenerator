...
class WordTreeGenerator:
    def __init__(self, word, file_name, file_output_name, content_column='Content', encoding='utf-8', sep='|'):
        self.word = word
    ...
    def read_data(self):
        if type(self.file_name) == str:
            self.data = pd.read_csv(self.file_name, sep=self.sep, encoding=self.encoding)
        else:
